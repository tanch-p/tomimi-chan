		...
		{#if $selectedChara}
			...
			<div class="px-1.5 mt-3">
				...
				{#if $selectedChara.tokens && $selectedChara.tokens.length > 0}
					{@const tokenToSkill1to1Linked =
						$selectedChara.tokens.length === $selectedChara.skills.length}
					{@const tokenToSkillManyLinked =
						$selectedChara.tokens[0].skills.length === $selectedChara.skills.length}
					<hr class="mt-4 mb-3 border-t-2 border-[#555]" />
					<p class="mt-6">
					...
					{#if tokenToSkill1to1Linked || tokenToSkillManyLinked}
						<div class="flex justify-evenly mt-6">
							{#each $selectedChara.skills as skill, skillIdx}
					...
					{/if}
					{#each $selectedChara.tokens as token, tokenIdx}
								...
								<div>
									...
									>
										{translations[language][
											`position_${getTokenPosition(token, $selectedChara.uniequip[moduleIndex])}`
										]}
									</div>
